...
class RosaRuntime:
    ...
    contract. It preserves exact latest-longest matching over the complete
    accumulated history; there is no suffix-window limit. Runtime slots are
    fixed by the first update; supplying ``sequence_ids`` on that update
    enables reorder checks.
    """
    ...
    def memory_stats(self) -> Dict[str, int]:
        ...
        ``logical_bytes`` counts stored SAM states, compact edges, optional
        transition/path indexes, and payload symbols. It excludes allocator
        capacity and container overhead, so it is stable across builds.
        """
        result = self._submit(self._native.stats).result()
        names = (
            "states",
    ...
    def complexity_stats(self) -> Dict[str, int]:
        """Return cumulative exact-automaton work counters.

        Maxima are per sequence/head automaton; all other fields are totals.
        The counters are observational and do not change routing semantics.
        """
        result = self._submit(self._native.complexity_stats).result()
        names = (
            "updates",
            "transition_probes",
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        raise NotImplementedError(
            ...
            "unlimited-suffix automata have no stable serialization schema"
        )
        ...
        try:
            ...
            if stream is None:
                ...
                with torch.cuda.stream(stream):
                    ...
                    try:
                        packed_output_cpu = torch.empty(
                            (query_cpu.size(0), self.num_heads),
                            dtype=torch.uint8,
                            device="cpu",
                            pin_memory=True,
                        )
                        matched_positions_cpu = torch.empty(
                            (query_cpu.size(0), self.num_heads),
                            dtype=torch.int64,
                            device="cpu",
                            pin_memory=True,
                        ...
                        packed_output_cpu = None
                        matched_positions_cpu = None
                ...
                def update_after_transfer():
                    ready.synchronize()
                    if packed_output_cpu is not None:
                        return self._native.update_packed_out(
                            offsets,
                            query_cpu,
                            key_cpu,
                            payload_cpu,
                            packed_output_cpu,
                            matched_positions_cpu,
                        )
                    packed_output, matched_positions = (
                    ...
                    return _try_pin(packed_output), _try_pin(matched_positions)
