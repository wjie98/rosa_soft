...
def test_packed_dense_matches_unbounded_reference():
...
def test_surrogate_horizon_does_not_change_runtime_or_training_hard_route():
    ...
    with rosa_soft.RosaRuntime(1, 1, 1, 3) as runtime:
        packed_output, end_positions = runtime.update_packed(
            query,
            key,
            payload,
        )
    expected = _unbounded_reference(query, key, payload)
    assert torch.equal(packed_output, expected[0])
    assert torch.equal(end_positions, expected[1])
...
def test_varlen_and_grouped_payload_heads_match_reference():
    ...
    heads, payload_heads, bits = 6, 3, 3
    total = sum(lengths)
...
def test_chunked_updates_equal_one_shot_update():
    batch, tokens, heads, payload_heads, bits = (
        2,
        24,
        4,
        2,
...
def test_packed_inputs_ignore_bits_above_declared_width():
    ...
    with rosa_soft.RosaRuntime(3, 1, 1, 2) as canonical:
        expected = canonical.update_packed(query, key, payload)
    with rosa_soft.RosaRuntime(3, 1, 1, 2) as noisy:
...
def test_cpu_async_updates_are_serialized_in_submission_order():
    ...
    with rosa_soft.RosaRuntime(1, 1, 1, 8) as runtime:
...
def test_async_submission_applies_bounded_backpressure(monkeypatch):
    ...
    runtime = rosa_soft.RosaRuntime(1, 1, 1, 1)
...
def test_reset_clears_state_stats_and_slot_binding():
    ...
    with rosa_soft.RosaRuntime(2, 1, 1, 3) as runtime:
        runtime.update_packed(
            packed,
            packed,
            payload,
...
def test_slot_count_is_fixed_until_reset():
    ...
    with rosa_soft.RosaRuntime(1, 1, 1, 1) as runtime:
        runtime.update_packed(packed, packed, packed)
...
def test_unary_history_stays_compressed_without_changing_routes():
    tokens = 8
    packed = torch.zeros(1, tokens, 1, dtype=torch.uint8)
    payload = torch.arange(tokens, dtype=torch.uint8).view(1, tokens, 1)
    with rosa_soft.RosaRuntime(1, 1, 1, 8) as runtime:
        actual = runtime.update_packed(packed, packed, payload)
        stats = runtime.complexity_stats()
        memory = runtime.memory_stats()

    expected = _unbounded_reference(packed, packed, payload)
    assert torch.equal(actual[0], expected[0])
    assert torch.equal(actual[1], expected[1])
    assert stats["updates"] == tokens
    assert stats["latest_end_updates"] == 0
    assert stats["compressed_run_symbols"] == tokens
    assert stats["compressed_materializations"] == 0
    assert stats["max_compressed_run_length"] == tokens
    assert stats["clones"] == 0
    assert memory["states"] == 1
    assert memory["edges"] == 0


def _diagonal_end_positions(query, key):
    query_codes = query.flatten().tolist()
    key_codes = key.flatten().tolist()
    previous = [0] * len(query_codes)
    end_positions = [-1] * len(query_codes)
    for query_end in range(1, len(query_codes)):
        current = [0] * len(query_codes)
        best_length = 0
        for key_end in range(query_end):
            if query_codes[query_end] == key_codes[key_end]:
                current[key_end + 1] = previous[key_end] + 1
            if current[key_end + 1] >= best_length and current[key_end + 1]:
                best_length = current[key_end + 1]
                end_positions[query_end] = key_end
        previous = current
    return torch.tensor(end_positions, dtype=torch.int64).view_as(query)


@pytest.mark.parametrize("query_seed", [None, 41])
def test_materialized_run_and_latest_path_tree_match_diagonal_reference(
    query_seed,
):
    key = torch.tensor(
        [0] * 96 + [1] * 192,
    ...
    query = (
        ...
        else _random_packed(key.shape, 2, seed=query_seed)
    )
    payload = torch.arange(key.size(1), dtype=torch.uint8).view_as(key)
    expected_ends = _diagonal_end_positions(query, key)

    with rosa_soft.RosaRuntime(1, 1, 2, 8) as runtime:
        _, actual_ends = runtime.update_packed(query, key, payload)
        stats = runtime.complexity_stats()

    assert torch.equal(actual_ends, expected_ends)
    assert stats["compressed_materializations"] == 1
    assert stats["latest_tree_activations"] == 1
    assert stats["latest_path_updates"] > 0
    assert stats["latest_tree_rotations"] > 0


def test_latest_path_tree_clone_reparent_is_exact_across_chunks():
    tail = [1, 2, 1, 3] * 80
    key = torch.tensor(
        [0] * 96 + tail,
        dtype=torch.uint8,
    ).view(1, -1, 1)
    query = _random_packed(key.shape, 2, seed=53)
    payload = (
        torch.arange(key.size(1), dtype=torch.int64)
        .to(torch.uint8)
        .view_as(key)
    )
    expected_ends = _diagonal_end_positions(query, key)
    ...
    pieces = []
    boundaries = (0, 17, 95, 143, 271, key.size(1))
    with rosa_soft.RosaRuntime(1, 1, 2, 8) as runtime:
    ...
    assert torch.equal(torch.cat(pieces, dim=1), expected_ends)
    assert stats["compressed_materializations"] == 1
    assert stats["latest_tree_activations"] == 1
    assert stats["latest_path_updates"] > 0
    assert stats["clones"] > 0
...
def test_long_chunked_history_preserves_global_successor_positions():
    # An all-identical stream is the suffix automaton's quadratic update
    # worst case.  This is still long enough to cross any former 32-token
    # proxy horizon while keeping the semantic regression test focused.
    chunk_tokens = 256
    heads = 2
...
def test_cuda_stream_async_matches_blocking():
    ...
    with rosa_soft.RosaRuntime(2, 1, 3, 3) as blocking:
        expected = blocking.update_packed(query, key, payload)

    stream = torch.cuda.Stream()
    with rosa_soft.RosaRuntime(2, 1, 3, 3) as asynchronous:
