#define rosa_soft_streaming_vjp_cuda rosa_soft_block_diagonal_baseline_vjp_cuda
...
constexpr int kPipelineGroupThreads = kBlockThreads / 2;
constexpr int kPipelineGroupWarps = kPipelineGroupThreads / kWarpSize;
...
constexpr int kBlockPipelineSharedItems =
    kBlockSharedItems + kBlockMatrixItems;
...
template <bool PipelineFirstPass = false>
__device__ __forceinline__ BlockShared bind_block_shared(float* storage) {
  ...
  float* stats = storage + kBlockPayloadItems +
      (PipelineFirstPass ? kBlockMatrixItems : 0);
  return {
      primary,
      secondary,
      ring,
...
// Barriers 1/2 synchronize one 128-thread role group. Barriers 3/4 transfer
// ready buffers and 5/6 transfer free buffers; producer and consumer arrivals
// together satisfy the 256-thread count.
__device__ __forceinline__ void pipeline_producer_sync() {
  asm volatile("bar.sync 1, 128;" : : : "memory");
}


__device__ __forceinline__ void pipeline_consumer_sync() {
  asm volatile("bar.sync 2, 128;" : : : "memory");
}


__device__ __forceinline__ void pipeline_producer_wait_free(int buffer) {
  if (buffer == 0) {
...
__device__ __forceinline__ void pipeline_producer_signal_ready(int buffer) {
  if (buffer == 0) {
...
__device__ __forceinline__ void pipeline_consumer_wait_ready(int buffer) {
  if (buffer == 0) {
...
__device__ __forceinline__ void pipeline_consumer_signal_free(int buffer) {
  if (buffer == 0) {
...
__device__ __forceinline__ void generate_block_scores_pipeline_group(
    const int32_t* __restrict__ query,
    const int32_t* __restrict__ key,
    ...
    float mismatch_unit,
    int local_thread) {
  for (int index = local_thread; index < kBlockMatrixItems;
       index += kPipelineGroupThreads) {
    output[index] = 0.0f;
  }
  pipeline_producer_sync();
  const int diagonal_count = row_count + route_count - 1;
  for (int diagonal = local_thread; diagonal < diagonal_count;
       diagonal += kPipelineGroupThreads) {
    const int difference = diagonal - (route_count - 1);
    const int first_row_offset = max(difference, 0);
    const int first_route_offset = max(-difference, 0);
    const int count = min(
    ...
    initialize_block_state(
        ...
        rolling_mismatch,
        rolling_count);
    output[first_row_offset * kBlockRoutes + first_route_offset] = score;
    for (int step = 1; step < count; ++step) {
      ++row;
      ++route;
      advance_block_state(
...
__device__ __forceinline__ float block_route_utility(
  ...
  for (int feature = 0; feature < value_dim; ++feature) {
    const float value_sign = read_float(value, value_base + feature) > 0.0f
        ? 1.0f
        : -1.0f;
    utility += read_float(grad_output, grad_base + feature) * value_sign;
  }
  return utility;
}
...
template <typename scalar_t>
__device__ __forceinline__ void generate_block_utilities(
    const scalar_t* __restrict__ value,
    const scalar_t* __restrict__ grad_output,
    BlockShared shared,
    int batch,
    ...
    int value_dim,
    bool needs_qk) {
  const int items = row_count * route_count;
...
template <typename scalar_t>
__device__ __forceinline__ void generate_block_utilities_pipeline_group(
    const scalar_t* __restrict__ value,
    const scalar_t* __restrict__ grad_output,
    BlockShared shared,
    int batch,
  ...
  float* grad_tile = shared.secondary + kBlockMatrixItems;
  float* value_tile = grad_tile + kBlockGradItems;
  for (int index = local_thread; index < kBlockGradItems;
       index += kPipelineGroupThreads) {
    const int item = index / kBlockValueDim;
    const int feature = index - item * kBlockValueDim;
    float grad = 0.0f;
  ...
  for (int index = local_thread; index < kBlockValueItems;
       index += kPipelineGroupThreads) {
    const int item = index / kBlockValueDim;
    const int feature = index - item * kBlockValueDim;
    float sign = 0.0f;
  ...
  pipeline_consumer_sync();

  const int local_warp = local_thread / kWarpSize;
  constexpr int kRouteTiles = kBlockRoutes / kTensorTile;
  constexpr int kUtilityTiles =
      (kBlockRows / kTensorTile) * kRouteTiles;
  for (int tile = local_warp; tile < kUtilityTiles;
       tile += kPipelineGroupWarps) {
    const int output_row = (tile / kRouteTiles) * kTensorTile;
    const int output_column = (tile % kRouteTiles) * kTensorTile;
    wmma::fragment<
        wmma::accumulator,
...
__device__ __forceinline__ void contract_block_symbol_vjp(
    const int32_t* __restrict__ query,
    const int32_t* __restrict__ key,
    BlockShared shared,
  ...
  const int query_base = row_start - kBlockWindow + 1;
  const int key_base = route_start - kBlockWindow;
  if ((gradient_mask & kGradQuery) != 0) {
    const int items = kBlockSpan * symbol_dim;
    for (int item = threadIdx.x; item < items; item += blockDim.x) {
      const int query_local = item / symbol_dim;
      const int bit = item - query_local * symbol_dim;
      const int query_position = query_base + query_local;
      if (query_position >= 0 && query_position < seq_len) {
        float contribution = 0.0f;
#pragma unroll 4
        for (int key_local = 0; key_local < kBlockSpan; ++key_local) {
          const int key_position = key_base + key_local;
          if (key_position >= 0 && key_position < seq_len) {
            contribution +=
                shared.secondary[
                    query_local * kBlockSpanStride + key_local] *
                static_cast<float>(sign_from_bit(
                    static_cast<uint32_t>(key[key_position]), bit));
          }
        }
        contribution *= derivative_scale;
        if (contribution != 0.0f) {
          const int64_t target =
  ...
  if ((gradient_mask & kGradKey) != 0) {
    const int items = kBlockSpan * symbol_dim;
    for (int item = threadIdx.x; item < items; item += blockDim.x) {
      const int key_local = item / symbol_dim;
      const int bit = item - key_local * symbol_dim;
      const int key_position = key_base + key_local;
      if (key_position >= 0 && key_position < seq_len) {
        float contribution = 0.0f;
#pragma unroll 4
        for (int query_local = 0; query_local < kBlockSpan; ++query_local) {
          const int query_position = query_base + query_local;
          if (query_position >= 0 && query_position < seq_len) {
            contribution +=
                shared.secondary[
                    query_local * kBlockSpanStride + key_local] *
                static_cast<float>(sign_from_bit(
                    static_cast<uint32_t>(query[query_position]), bit));
          }
        }
        contribution *= derivative_scale;
        if (contribution != 0.0f) {
...
template <typename scalar_t, bool UseTensor, bool PipelineFirstPass = false>
...
void block_diagonal_vjp_kernel(
    ...
    int gradient_mask) {
  static_assert(!PipelineFirstPass || UseTensor);
  static_assert(!PipelineFirstPass || kBlockThreads == 256);
  extern __shared__ float storage[];
  const BlockShared shared = bind_block_shared<PipelineFirstPass>(storage);
  ...
  const int pipeline_group = threadIdx.x / kPipelineGroupThreads;
  const int pipeline_thread = threadIdx.x % kPipelineGroupThreads;
  const int stats_row = PipelineFirstPass ? pipeline_thread : threadIdx.x;
  const bool owns_stats_row = PipelineFirstPass
      ? pipeline_group == 1 && stats_row < row_count
      : stats_row < row_count;

  SoftmaxStats row_stats = owns_stats_row
      ? SoftmaxStats{kNullScore * scale, 1.0f, 0.0f}
      : SoftmaxStats{-FLT_MAX, 0.0f, 0.0f};
  if constexpr (PipelineFirstPass) {
    float* score_buffers[2] = {
        shared.primary,
        storage + kBlockPayloadItems};
    const int route_tiles =
        (row_end + kBlockRoutes - 1) / kBlockRoutes;
    if (pipeline_group == 0) {
      for (int tile = 0; tile < route_tiles; ++tile) {
        const int buffer = tile & 1;
        if (tile >= 2) {
          pipeline_producer_wait_free(buffer);
        }
        const int route_start = 1 + tile * kBlockRoutes;
        const int route_count = min(kBlockRoutes, seq_len - route_start);
        generate_block_scores_pipeline_group(
            query,
            key,
            score_buffers[buffer],
            row_start,
            row_count,
            route_start,
            route_count,
            ...
            mismatch_unit,
            pipeline_thread);
        pipeline_producer_signal_ready(buffer);
      }
    } else {
      for (int tile = 0; tile < route_tiles; ++tile) {
        const int buffer = tile & 1;
        pipeline_consumer_wait_ready(buffer);
        const int route_start = 1 + tile * kBlockRoutes;
        const int route_count = min(kBlockRoutes, seq_len - route_start);
        generate_block_utilities_pipeline_group(
            value,
            grad_output,
            shared,
            batch,
        ...
        if (owns_stats_row) {
          ...
          for (int route_offset = 0; route_offset < route_count;
               ++route_offset) {
            const int route = route_start + route_offset;
            if (route <= row) {
              const float raw_score = score_buffers[buffer][
        ...
        pipeline_consumer_sync();
        pipeline_consumer_signal_free(buffer);
      }
    }
    __syncthreads();
  } else {
  ...
  for (int route_start = 1; route_start <= row_end;
    ...
    if constexpr (UseTensor) {
      ...
      const int value_items = route_count * value_dim;
...
rosa_soft_block_diagonal_vjp_cuda(
    ...
    int gradient_mask,
    int plan) {
  const bool tensor_plan_unavailable =
      plan >= 2 &&
      (value.size(3) != kBlockValueDim ||
       at::cuda::getCurrentDeviceProperties()->major < 8);
  const bool pipeline_plan_unavailable =
      plan == 3 && kBlockThreads != 256;
  if (plan == 0 || max_suffix_length != kBlockWindow ||
      tensor_plan_unavailable || pipeline_plan_unavailable) {
    return rosa_soft_block_diagonal_baseline_vjp_cuda(
        query,
        key,
        value,
        grad_output,
  ...
  const bool use_pipeline = plan == 3;
  const size_t shared_bytes =
      (use_pipeline ? kBlockPipelineSharedItems : kBlockSharedItems) *
      sizeof(float);
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  const float inverse_keep_probability = 1.0f / (1.0f - dropout_p);
  const bool use_tensor = plan >= 2;
  ...
  DISPATCH_ROSA_FLOAT_TYPES(
      ...
      [&] {
        if (use_tensor) {
#if ROSA_BLOCK_THREADS == 256
          if (use_pipeline) {
            C10_CUDA_CHECK(cudaFuncSetAttribute(
                block_diagonal_vjp_kernel<scalar_t, true, true>,
          ...
          {
          C10_CUDA_CHECK(cudaFuncSetAttribute(
              block_diagonal_vjp_kernel<scalar_t, true>,
              cudaFuncAttributeMaxDynamicSharedMemorySize,
              static_cast<int>(shared_bytes)));
          block_diagonal_vjp_kernel<scalar_t, true><<<
              static_cast<int>(blocks64),
          ...
          C10_CUDA_CHECK(cudaFuncSetAttribute(
              block_diagonal_vjp_kernel<scalar_t, false>,
              cudaFuncAttributeMaxDynamicSharedMemorySize,
              static_cast<int>(shared_bytes)));
          block_diagonal_vjp_kernel<scalar_t, false><<<
              static_cast<int>(blocks64),
