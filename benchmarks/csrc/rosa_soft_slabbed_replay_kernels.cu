...
// Capacity is private and fixed: it changes storage granularity, never suffix
// horizon or candidate coverage.
...
__global__ void fused_stats_partial_fp16_kernel(
  ...
  __shared__ float score_tile[kFusedRows * kFusedDiagonals];
  ...
  const int local_diagonal = thread & (kFusedDiagonals - 1);
  const int diagonal_start = 1 + diagonal_tile * kFusedDiagonals;
  const int delta = diagonal_start + local_diagonal;
  ...
  for (int index = thread;
       index < kFusedRoutes * kFusedValueDim;
       index += blockDim.x) {
  ...
  if (thread < kFusedDiagonals) {
    float score = delta < seq_len
        ? score_carry[series_offset + delta]
        : 0.0f;
#pragma unroll
    for (int row_offset = 0; row_offset < kFusedRows; ++row_offset) {
      const int row = row_start + row_offset;
      const bool active = row_offset < row_count && delta <= row;
      if (active) {
        ...
        score = fmaf(gate, score, gate);
      }
      score_tile[row_offset * kFusedDiagonals + local_diagonal] =
          active ? score : 0.0f;
    }
    if (delta < seq_len) {
      score_carry[series_offset + delta] = score;
    }
  }
  __syncthreads();
...
#pragma unroll
  ...
  const int reduction_group = thread / kWarpSize;
  const int reduction_lane = thread & (kFusedDiagonals - 1);
  for (int row_base = 0;
       row_base < kFusedRows;
       row_base += kFusedThreads / kWarpSize) {
    const int row_offset = row_base + reduction_group;
    const int row = row_start + row_offset;
    const int candidate_delta = diagonal_start + reduction_lane;
    const bool active = row_offset < row_count && candidate_delta <= row;
    SoftmaxStats item = {-FLT_MAX, 0.0f, 0.0f};
    if (active) {
      const int route_offset =
          row_offset - reduction_lane + kFusedDiagonals - 1;
      const int route = route_start + route_offset;
      const ScoreTransform transformed = transform_score(
          score_tile[row_offset * kFusedDiagonals + reduction_lane]);
      const float utility = attention_dropout_scale(
          dropout_seed,
          dropout_p,
          ...
          head,
          row,
          route) * utility_tile[row_offset * kFusedRoutes + route_offset];
      item = {
          transformed.route_score * scale - logf(static_cast<float>(row)),
          1.0f,
          utility};
    }
    item = warp_reduce_stats(item);
    if (reduction_lane == 0 && row_offset < row_count) {
      const int64_t target =
          (((static_cast<int64_t>(series) * kFusedRows + row_offset) *
                maximum_diagonal_tiles +
            diagonal_tile) *
           kRowStatsWidth);
...
__global__ void reduce_fused_stats_kernel(
    ...
    int maximum_diagonal_tiles,
    float scale) {
  const int local_row = blockIdx.x % row_count;
  const int series = blockIdx.x / row_count;
  if (series >= series_count) {
    return;
  }
  SoftmaxStats local = threadIdx.x == 0
      ? SoftmaxStats{kNullScore * scale, 1.0f, 0.0f}
      : SoftmaxStats{-FLT_MAX, 0.0f, 0.0f};
  for (int tile = threadIdx.x;
       tile < diagonal_tile_count;
       tile += blockDim.x) {
    const int64_t source =
        (((static_cast<int64_t>(series) * kFusedRows + local_row) *
              maximum_diagonal_tiles +
          tile) *
         kRowStatsWidth);
  ...
  __syncthreads();
  if (warp == 0) {
    SoftmaxStats total = lane < kWarpsPerBlock
        ? warp_stats[lane]
        : SoftmaxStats{-FLT_MAX, 0.0f, 0.0f};
    total = warp_reduce_stats(total);
    if (lane == 0) {
      const int row = row_start + local_row;
      const int64_t target =
          (static_cast<int64_t>(series) * seq_len + row) * kRowStatsWidth;
      const float inverse_normalizer = 1.0f / total.normalizer;
      row_stats[target + kRowMaximum] = total.maximum;
      row_stats[target + kRowNormalizer] = inverse_normalizer;
      row_stats[target + kRowUtility] =
          total.utility_numerator * inverse_normalizer;
    }
  }
}
...
__global__ void fused_slab_stats_fp16_kernel(
  ...
  const int delta = diagonal_start + lane;
  const int head = series % num_heads;
  const int batch = series / num_heads;
  const int value_head = head / (num_heads / num_value_heads);
  const int64_t series_offset = static_cast<int64_t>(series) * seq_len;
  ...
  for (int row_start = first_row_start;
    ...
    if (warp == 0 && lane < kFusedDiagonals) {
#pragma unroll
      for (int row_offset = 0; row_offset < kFusedRows; ++row_offset) {
        const int row = row_start + row_offset;
        ...
        score_tile[row_offset * kFusedDiagonals + lane] =
            active ? score : 0.0f;
      }
    }
    __syncthreads();
...
#pragma unroll
    ...
    for (int reduction_row = warp;
      ...
      if (active) {
        ...
        const int route = route_start + route_offset;
        const ScoreTransform transformed = transform_score(
            score_tile[reduction_row * kFusedDiagonals + lane]);
        const float utility = attention_dropout_scale(
            dropout_seed,
            dropout_p,
            inverse_keep_probability,
...
__global__ void merge_fused_slab_stats_kernel(
  ...
  const int active_tiles = min(
      diagonal_tile_count,
      (row - slab_start) / kFusedDiagonals + 1);
...
template <typename scalar_t, int kDiagonalCount>
__global__ void fused_slab_reverse_fp16_kernel(
  ...
  constexpr int kRouteCount =
      ((kFusedRows + kDiagonalCount - 1 + kTensorTile - 1) /
       kTensorTile) * kTensorTile;
  constexpr int kWarpCount = kFusedThreads / kWarpSize;
  constexpr int kDiagonalRounds = kDiagonalCount / kWarpCount;
  __shared__ __align__(16) __half grad_tile[
      kFusedRows * kFusedValueDim];
  __shared__ __align__(16) __half value_tile[
  ...
  __shared__ float score_tile[kDiagonalCount * kScoreStride];
  ...
  const int warp = thread / kWarpSize;
  const int lane = thread & (kWarpSize - 1);
  const int local_diagonal_start = diagonal_tile * kDiagonalCount;
  const int diagonal_start = slab_start + local_diagonal_start;
  const int tile_width = min(
      kDiagonalCount, slab_width - local_diagonal_start);
  const int head = series % num_heads;
  const int batch = series / num_heads;
  const int value_head = head / (num_heads / num_value_heads);
  const int64_t series_offset = static_cast<int64_t>(series) * seq_len;
  ...
  for (int row_start =
    ...
    for (int linear = thread;
         linear < kFusedRows * kDiagonalCount;
         linear += blockDim.x) {
    ...
    for (int diagonal_round = 0;
      ...
      const int mismatch = active
          ? __popc(
                (static_cast<uint32_t>(
                     packed_query[series_offset + row]) ^
                 static_cast<uint32_t>(packed_key[
                     series_offset + row - delta])) &
                mask)
          : 0;
      const float gate = active
          ? __expf(-mismatch_unit * static_cast<float>(mismatch))
      ...
      if (active) {
        ...
        const float probability = __expf(
            ...
            row_stats[stats_index + kRowNormalizer];
        const int route_offset =
            lane - diagonal_offset + kDiagonalCount - 1;
        const int route = route_start + route_offset;
        const float dropout_scale = attention_dropout_scale(
            dropout_seed,
            dropout_p,
...
template <typename scalar_t, int kRouteTile>
__global__ void tensor_slab_value_vjp_fp16_kernel(
  ...
  __shared__ __align__(16) float output_tile[
      kRouteTile * kFusedValueDim];

  const int route_tiles =
      (seq_len - slab_start + kRouteTile - 1) / kRouteTile;
  ...
  const int route_start = 1 + route_tile * kRouteTile;
  const int heads_per_value = num_heads / num_value_heads;
  const int warp = threadIdx.x / kWarpSize;
  ...
  const int final_query = min(
      seq_len - 1,
      route_start + kRouteTile - 1 + slab_start + slab_width - 2);
  ...
  for (int head_offset = 0;
    ...
    for (int query_start = first_query_tile;
      ...
      for (int index = threadIdx.x;
           index < kTensorTile * kRouteTile;
           index += kValueThreads) {
        const int query_offset = index / kRouteTile;
        const int route_offset = index - query_offset * kRouteTile;
      ...
      wmma::fragment<
          ...
          gradient;
      wmma::load_matrix_sync(
          probability, probability_high + output_route, kRouteTile);
      wmma::load_matrix_sync(
          gradient,
      ...
      wmma::load_matrix_sync(
          probability, probability_low + output_route, kRouteTile);
      wmma::mma_sync(accumulator, probability, gradient, accumulator);
      __syncthreads();
    }
  }
  ...
  __syncthreads();
  for (int index = threadIdx.x;
       index < kRouteTile * kFusedValueDim;
       index += kValueThreads) {
    const int route_offset = index / kFusedValueDim;
    const int feature = index - route_offset * kFusedValueDim;
    const int route = route_start + route_offset;
  ...
  DISPATCH_ROSA_FLOAT_TYPES(
      ...
      [&] {
        for (int slab_start = 1;
          ...
          fused_slab_stats_fp16_kernel<scalar_t><<<
...
rosa_soft_slabbed_replay_vjp_cuda(
  ...
  const bool use_fused_stats =
      fused_shape_supported && (execution_plan & 1) != 0;
  const bool use_fused_reverse =
      fused_shape_supported && (execution_plan & 2) != 0;
  const bool use_tensor_value =
      tensor_shape_supported && (execution_plan & 4) != 0;
  const bool use_tiled_symbol =
      needs_symbols && (execution_plan & 8) != 0;
          ...
          if ((gradient_mask & kGradValue) != 0) {
            if (use_tensor_value) {
              const int value_route_tiles =
                  (seq_len - slab_start + value_tile_size - 1) /
                  value_tile_size;
              if (value_tile_size == 32) {
                tensor_slab_value_vjp_fp16_kernel<scalar_t, 32><<<
                ...
                tensor_slab_value_vjp_fp16_kernel<scalar_t, 16><<<
                    batch_size * num_value_heads * value_route_tiles,
                    4 * kWarpSize,
                    0,
                    stream>>>(
          ...
          if (needs_symbols) {
            if (use_fused_reverse) {
              const int fused_diagonal_tiles =
                  (slab_width + diagonal_tile_size - 1) /
                  diagonal_tile_size;
              if (diagonal_tile_size == 16) {
                fused_slab_reverse_fp16_kernel<scalar_t, 16><<<
                ...
                fused_slab_reverse_fp16_kernel<scalar_t, 32><<<
                    series_count * fused_diagonal_tiles,
                    kFusedThreads,
                    0,
                    stream>>>(
