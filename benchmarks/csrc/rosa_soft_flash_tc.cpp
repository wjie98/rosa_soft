...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> flash_tc_vjp(
    const torch::Tensor& query,
    const torch::Tensor& key,
    const torch::Tensor& value,
    const torch::Tensor& grad_output,
  ...
  TORCH_CHECK(
      ...
      "packed query symbols must have shape [B, H, T]");
  TORCH_CHECK(
      packed_key_symbols.sizes() == packed_query_symbols.sizes(),
      "packed key symbols must match packed query symbols");
  ...
  TORCH_CHECK(plan >= 0 && plan <= 6);

  return rosa_soft_flash_tc_vjp_cuda(
      query,
      key,
      value,
      grad_output,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def(
      "flash_tc_vjp",
      &flash_tc_vjp,
      "Two-pass FlashROSA Tensor-Core VJP research plans (CUDA)");
}
