...
class RosaGSSFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        xq = _quantize(query, num_q_bits)
        xk = _quantize(key, num_k_bits)
        xv = _quantize(value, num_v_bits)
        ...
        r = torch.arange(num_v_bits, device=value.device)
        xo = ((xo.view(bsz, num_q_heads, seq_len, 1) >> r) & 1).to(value.dtype)
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        with torch.enable_grad():
            ...
            xq = xq[row].view(-1, 1, num_q_bits)
            xk = xk[col].view(-1, num_k_bits, 1)

            logits = (xq @ xk).view(-1) * alpha + attn_bias * beta
            ...
            xo = xo.index_add(0, row, xv[col] * exp_qk.view(-1, 1))

            xw = torch.zeros((num_nodes,), dtype=exp_qk.dtype, device=exp_qk.device)
            xw = xw.index_add(0, row, exp_qk)

            output = xo / xw.view(-1, 1)
...
def _normalize(x: Tensor, eps: float = 1e-6) -> Tensor:
    _x = x.float()
    _x = _x * torch.rsqrt(_x.square().sum(-1, keepdim=True) + eps)
    return _x.type_as(x)

def _quantize(x: Tensor, num_bits: int) -> Tensor:
