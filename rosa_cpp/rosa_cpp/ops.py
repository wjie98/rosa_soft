...
def rosa_trit_init(ctx: Tensor) -> Tensor:
    return torch.ops.rosa_cpp.rosa_trit_init(ctx)

def rosa_trit_free(ctx: Tensor) -> Tensor:
    return torch.ops.rosa_cpp.rosa_trit_free(ctx)

def rosa_trit_update(ctx: Tensor, q: Tensor, m: Tensor, k: Tensor, v: Tensor, u: int) -> Tensor:
    return torch.ops.rosa_cpp.rosa_trit_update(ctx, q, m, k, v, u)

def rosa_trit_inspect(ctx: Tensor, q: Tensor, m: Tensor, k: Tensor, v: Tensor, u: int) -> Tuple[Tensor, Tensor, Tensor]:
    return torch.ops.rosa_cpp.rosa_trit_inspect(ctx, q, m, k, v, u)

@torch.library.register_fake("rosa_cpp::rosa_trit_init")
def _(ctx: Tensor):
    torch._check(ctx.dim() == 1)
    torch._check(ctx.dtype == torch.long)
    return torch.empty_like(ctx)

@torch.library.register_fake("rosa_cpp::rosa_trit_free")
def _(ctx: Tensor):
    torch._check(ctx.dim() == 1)
    torch._check(ctx.dtype == torch.long)
    return torch.empty_like(ctx)

@torch.library.register_fake("rosa_cpp::rosa_trit_update")
def _(ctx: Tensor, q: Tensor, k: Tensor, v: Tensor, u: int):
    torch._check(ctx.dim() == 1)
    torch._check(ctx.dtype == torch.long)
...
@torch.library.register_fake("rosa_cpp::rosa_trit_inspect")
def _(q: Tensor, k: Tensor, v: Tensor, u: int):
    torch._check(q.dim() == 2)
    torch._check(q.dtype == torch.long)
...
def rosa_trit_forward(q: Tensor, m: Tensor, k: Tensor, v: Tensor, u: int) -> Tensor:
    return torch.ops.rosa_cpp.rosa_trit_forward(q, m, k, v, u)

@torch.library.register_fake("rosa_cpp::rosa_trit_forward")
def _(q: Tensor, k: Tensor, v: Tensor, u: int):
    torch._check(q.dim() == 2)
    torch._check(q.dtype == torch.long)
