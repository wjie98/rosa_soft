...
__global__ void rwkv7_clampw_forward_kernel(
    int T, int H,
...
template<typename F>
void rwkv7_clampw_cuda_forward(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, F* r, F* w, F* k, F* v, F* a, F* b, F* y, float* s, float* sa)
{
    DISPATCH_HEAD_SIZE(head_size, [&] {
        DISPATCH_CHUNK_LEN(chunk_len, [&] {
            rwkv7_clampw_forward_kernel<F, HEAD_SIZE, CHUNK_LEN><<<dim3(num_heads, batch_size), dim3(HEAD_SIZE)>>>(seq_len, num_heads, r, w, k, v, a, b, y, s, sa);
        });
    });
}
...
__global__ void rwkv7_clampw_backward_kernel(
    int T, int H,
    F* __restrict__ r_, F* __restrict__ w_, F* __restrict__ k_, F* __restrict__ v_,
    F* __restrict__ a_, F* __restrict__ b_, F* __restrict__ dy_, float* __restrict__ s__, float* __restrict__ sa_,
...
template<typename F>
void rwkv7_clampw_cuda_backward(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, F* r, F* w, F* k, F* v, F* a, F* b, F* dy, float* s, float* sa, F* dr, F* dw, F* dk, F* dv, F* da, F* db)
{
    DISPATCH_HEAD_SIZE(head_size, [&] {
        DISPATCH_CHUNK_LEN(chunk_len, [&] {
            rwkv7_clampw_backward_kernel<F, HEAD_SIZE, CHUNK_LEN><<<dim3(num_heads, batch_size), dim3(HEAD_SIZE)>>>(seq_len, num_heads, r, w, k, v, a, b, dy, s, sa, dr, dw, dk, dv, da, db);
        });
    });
}

//###################################################################################################### 
template void rwkv7_clampw_cuda_forward<float>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, float* r, float* w, float* k, float* v, float* a, float* b, float* y, float* s, float* sa);
template void rwkv7_clampw_cuda_forward<__half>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, __half* r, __half* w, __half* k, __half* v, __half* a, __half* b, __half* y, float* s, float* sa);
template void rwkv7_clampw_cuda_forward<__nv_bfloat16>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, __nv_bfloat16* r, __nv_bfloat16* w, __nv_bfloat16* k, __nv_bfloat16* v, __nv_bfloat16* a, __nv_bfloat16* b, __nv_bfloat16* y, float* s, float* sa);

template void rwkv7_clampw_cuda_backward<float>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, float* r, float* w, float* k, float* v, float* a, float* b, float* dy, float* s, float* sa, float* dr, float* dw, float* dk, float* dv, float* da, float* db);
template void rwkv7_clampw_cuda_backward<__half>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, __half* r, __half* w, __half* k, __half* v, __half* a, __half* b, __half* dy, float* s, float* sa, __half* dr, __half* dw, __half* dk, __half* dv, __half* da, __half* db);
template void rwkv7_clampw_cuda_backward<__nv_bfloat16>(int batch_size, int seq_len, int num_heads, int head_size, int chunk_len, __nv_bfloat16* r, __nv_bfloat16* w, __nv_bfloat16* k, __nv_bfloat16* v, __nv_bfloat16* a, __nv_bfloat16* b, __nv_bfloat16* dy, float* s, float* sa, __nv_bfloat16* dr, __nv_bfloat16* dw, __nv_bfloat16* dk, __nv_bfloat16* dv, __nv_bfloat16* da, __nv_bfloat16* db);
