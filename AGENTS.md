...
- backward evaluates every valid causal action and differentiates the full
  dense Q/K proxy distribution;
- V receives probability-weighted credit from the same dense distribution.
...
which actions participate in the default backward pass.
...
- online softmax reductions over all causal actions;
- FlashAttention-style dense action tiling;
...
Every valid causal action must still be visited. Numerical underflow is a
floating-point issue; structurally masking or omitting an action is not
allowed.
...
Distributed soft V credit is part of the validated estimator. Hard-selected V
won a direct structural probe but failed the repeated-motif full-model fit for
all tested lambda values, so it must remain research-only unless a replacement
passes both gates.

Do not reintroduce dynamic schedules, window-derived lambda, confidence
weighting, winner margins, recency bias, configurable perturbation modes,
value-gradient modes, early-stop epsilon, Q/K dampers, or hot-path telemetry
...
- bit-exact hard forward output and latest-action tie behavior;
